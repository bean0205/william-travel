...
export const getPermissions = async (skip = 0, limit = 100): Promise<Permission[]> => {
  const response = await axios.get(API_ENDPOINTS.permissions.list, {
    params: { skip, limit },
  });
...
export const getPermissionById = async (permissionId: number): Promise<Permission> => {
  const response = await axios.get(API_ENDPOINTS.permissions.detail(permissionId));
  return response.data;
};

// Create a new permission
export const createPermission = async (payload: PermissionCreatePayload): Promise<Permission> => {
  const response = await axios.post(API_ENDPOINTS.permissions.list, payload);
  return response.data;
};

// Update a permission
export const updatePermission = async (permissionId: number, payload: PermissionUpdatePayload): Promise<Permission> => {
  const response = await axios.put(API_ENDPOINTS.permissions.detail(permissionId), payload);
  return response.data;
};

// Delete a permission
export const deletePermission = async (permissionId: number): Promise<void> => {
  await axios.delete(API_ENDPOINTS.permissions.detail(permissionId));
};
