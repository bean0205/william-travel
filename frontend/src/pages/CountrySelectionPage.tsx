      ...
      <main className="relative z-10 mx-auto flex w-full max-w-7xl flex-col px-4 py-20 md:py-24 flex-grow pb-24">
        {!selectedContinent ? (
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-12">
            ...
            {/* Continent grid */}
            <div className="col-span-1 grid grid-cols-1 gap-4 sm:grid-cols-2 md:col-span-2 lg:col-span-3 xl:col-span-8 xl:grid-cols-3">
              {continents.map((continent, index) => (
                <div
                  key={continent.id}
                  className="group relative flex cursor-pointer flex-col overflow-hidden rounded-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-slide-in"
            ...
            {selectedCountryCard && (
              <div className="mx-auto w-full max-w-md">
                <Button
                  ...
                  onClick={() => {
                    const country = countries.find((c) => c.code === selectedCountryCard);
                    if (country) handleCountrySelect(country);
                  }}
                >
                  <span className="flex items-center justify-center">
                    <MapPin className="mr-2 h-4 w-4" />
                    <span>{t('buttons.continueWith')} {countries.find((c) => c.code === selectedCountryCard)?.name}</span>
                  </span>
                </Button>
              </div>
            )}
