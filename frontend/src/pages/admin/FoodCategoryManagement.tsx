...
import {
  FoodCategory, getFoodCategories, getFoodCategoryById,
  createFoodCategory, updateFoodCategory
...
const FoodCategoryManagement: React.FC = () => {
  // State
  ...
  const [viewCategory, setViewCategory] = useState<FoodCategory | null>(null);
  ...
  const showEditModal = async (category: FoodCategory) => {
    try {
      setLoading(true);
      const detailedCategory = await getFoodCategoryById(category.id);
  ...
  const showViewModal = async (categoryId: number) => {
    try {
      setLoading(true);
      const category = await getFoodCategoryById(categoryId);
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (modalType === 'create') {
        ...
        await updateFoodCategory(editingCategory.id, values);
  ...
  const columns = [
    {
      ...
      dataIndex: 'id',
      key: 'id',
      sorter: (a: FoodCategory, b: FoodCategory) => a.id - b.id,
    },
    {
      title: 'Name',
      dataIndex: 'name',
      key: 'name',
      sorter: (a: FoodCategory, b: FoodCategory) => a.name.localeCompare(b.name),
    },
    {
      title: 'Description',
    ...
    },
    {
      ...
      onFilter: (value: number, record: FoodCategory) => record.status === value,
    },
    {
      title: 'Created',
    ...
    },
    {
      title: 'Actions',
      key: 'actions',
      render: (_: any, record: FoodCategory) => (
        <Space size="middle">
          <Button
            type="text"
            icon={<EyeOutlined />}
  ...
  return (
    <div className="food-category-management-container">
      <Card>
        <div className="category-management-header" style={{ marginBottom: 16 }}>
          <Row justify="space-between" align="middle">
            <Col>
              <Title level={2}>Food Categories</Title>
            </Col>
            <Col>
              <Space>
                <Search
      ...
      >
        <Form form={form} layout="vertical">
          ...
          >
            <Input prefix={<CoffeeOutlined />} placeholder="Category Name" />
          </Form.Item>
